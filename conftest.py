...
def example_data_products(request, app, db):
    with app.app_context():
        ...
        p1 = m.Product(
            name={"en": "Multilingual Product", "de": "Mehrsprachiges Produkt"},
            details={"en": "A product of society", "de": "Ein Produkt der Gesellschaft"},
        )
        i1 = m.Ingredient(
            weight=1000,
            name={"en": "Multilingual Ingredient", "de": "Mehrsprachige Ingredient"},
            product=p1,
        )

        p2 = m.Product(
            name={"en": "English Product"}, details={"en": "A product of English society"}
        )
        i2 = m.Ingredient(weight=1000, name={"en": "English Ingredient (probably Tea)"}, product=p2)

        p3 = m.Product(name={"de": "Deutsches Produkt"}, details={"de": "Faelscht Emissionsdaten"})
        i3 = m.Ingredient(
            weight=10000,
...
def example_data_criteria(request, app, db):
    with app.app_context():
        ...
        c1 = m.Criterion(
            name={"en": "Multilingual Criterion", "de": "Mehrsprachiges Criterion"},
            details={"en": "Multilingual details", "de": "Mehrsprachige Details"},
        )
        c2 = m.Criterion(name={"en": "English Criterion"}, details={"en": "English details"})
        c3 = m.Criterion(name={"de": "Deutsches Criterion"}, details={"de": "Deutsche Details"})

        cc1 = m.CriterionCategory(
            name={"en": "Multilingual Criterion Category", "de": "Mehrsprachige Criterion Category"}
        )
...
def example_data_resources(request, app, db):
    with app.app_context():
        ...
        r3 = m.Resource(
            name={"en": "Multilingual Test Resource", "de": "Mehrsprachige Test Resource"}
        )
...
def example_data_hotspots(request, app, db):
    with app.app_context():
        ...
        h1 = m.Hotspot(
            name={"en": "Multilingual hotspot", "de": "Mehrsprachiger hotspot"},
            description={
        ...
        h2 = m.Hotspot(
            name={"en": "English hotspot"}, description={"en": "This hotspot speaks English"}
        )
        h3 = m.Hotspot(
            name={"de": "Deutscher hotspot"}, description={"de": "Dieser hotspot spricht deutsch"}
        )
...
def example_data_label_guide(request, app, db):
    with app.app_context():
        print("\nSetting up example data for label guide use cases for {} {}".format(id(db), db))

        l1 = m.Label(
            name={"en": "English and German", "de": "Englisch und Deutsch"},
            description={"en": "English description", "de": "Deutsche Beschreibung"},
            logo={"en": "english_logo.png", "de": "german_logo.png"},
        )
        l2 = m.Label(
        ...
        l3_m_c3 = m.LabelMeetsCriterion(
            label=l3, criterion=crit3, explanation={"de": "Alles nur Deutsch."}, score=100
        )
...
def example_data_labels(request, app, db):
    with app.app_context():
        ...
        hotspot = m.Hotspot(
            ...
            description={
                "en": "To achieve a better world, we need good code."
                + "And good code needs good QA."
            },
        )
        crit_hs = m.CriterionImprovesHotspot(
            weight=100, explanation={"en": "What better QA than solid test data?"}, hotspot=hotspot
        )
        crit = m.Criterion(
            type="label", name={"en": "The test improvement criterion"}, improves_hotspots=[crit_hs]
        )
        crit_cat = m.CriterionCategory(name={"en": "Test Data"}, criteria=[crit])
        lbl = m.Label(
            name={"en": "Testlabel"},
            type="product",
            description={"en": "For exceptional testing."},
            logo={"en": "beautiful_logo.png", "de": "horrible_logo.png"},
            resources=[r1, r2],
